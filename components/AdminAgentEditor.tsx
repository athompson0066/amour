...
import { Save, X, UserCheck, Image, DollarSign, BrainCircuit, Activity, Plus, Trash2, Loader2, Sparkles, Code2, Copy, Check, Info, Share2, Zap, ExternalLink, CircleDollarSign, Globe, Eye, Brain, Search, Link as LinkIcon, FileText, Database, Stars, Users, Terminal, Key } from 'lucide-react';
                ...
                <div className="lg:col-span-4 p-10 bg-slate-50/80 border-r border-slate-100 flex flex-col items-center">
                    ...
                    <div className="w-full space-y-6">
                        <div>
                            ...
                            <input type="text" value={avatar} onChange={(e) => setAvatar(e.target.value)} className="w-full bg-white border border-slate-200 rounded-2xl px-4 py-2.5 text-xs focus:ring-2 focus:ring-rose-500 outline-none transition-all" />
                        </div>
                        ...
                        {initialAgent && (
                            <div className="mt-8 p-6 bg-slate-900 rounded-[2rem] border border-slate-800 shadow-2xl overflow-hidden relative group">
                                <div className="absolute top-0 right-0 p-4 opacity-10"><Zap className="text-rose-500" size={60} /></div>
                                <div className="flex items-center justify-between mb-6 relative z-10"><div className="flex items-center space-x-2"><Share2 className="text-rose-500" size={16} /><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Live Integrations</label></div></div>
                                <div className="space-y-4 relative z-10">
                                    <div onClick={() => copyToClipboard(`[${embedCode || initialAgent.id}]`, 'slug')} className="group flex items-center justify-between bg-black/40 p-4 rounded-2xl border border-white/5 cursor-pointer hover:border-rose-500/50 transition-all active:scale-95"><div className="flex flex-col"><span className="text-[9px] text-slate-500 font-bold mb-1 uppercase">Directory Shortcode</span><code className="text-[11px] text-rose-400 font-mono">[{embedCode || 'id'}]</code></div>{copied === 'slug' ? <Check size={16} className="text-emerald-400" /> : <Copy size={16} className="text-slate-600 group-hover:text-white" />}</div>
                                    <div onClick={() => copyToClipboard(getExternalEmbedCode(), 'html')} className="group flex items-center justify-between bg-black/40 p-4 rounded-2xl border border-white/5 cursor-pointer hover:border-indigo-500/50 transition-all active:scale-95"><div className="flex flex-col"><span className="text-[9px] text-slate-500 font-bold mb-1 uppercase">External HTML Embed</span><code className="text-[11px] text-indigo-300 font-mono">&lt;iframe...&gt;</code></div>{copied === 'html' ? <Check size={16} className="text-emerald-400" /> : <Terminal size={16} className="text-slate-600 group-hover:text-white" />}</div>
                                </div>
                            </div>
                        )}
                    </div>
